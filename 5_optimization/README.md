...
|--------|---------|---------------|
| [`gradient_descent.py`](./gradient_descent.py) | `θ ← θ - η ∇L` | Descent on `(w-3)²` + learning-rate effect (saves a plot) |
| [`chain_rule_backprop.py`](./chain_rule_backprop.py) | `d/dx f(g(x)) = f'(g(x))g'(x)` | Autograd vs analytic gradients |
| [`adam_optimizer.py`](./adam_optimizer.py) | `θ ← θ - η m̂/(√v̂+ε)` | Adam by hand vs `torch.optim.Adam` |
| [`l2_regularization.py`](./l2_regularization.py) | `L_total = L + λΣθ²` | Weight decay shrinks the weights |
